...
raster_vacio <- rast(extent = c(extent_rejilla["xmin"], extent_rejilla["xmax"], extent_rejilla["ymin"], extent_rejilla["ymax"]), 
...
bloques  <- paste0(cut(train_sf$x_utm, breaks_x, labels=F, include.lowest=T), "_", 
...
resultados_cv <- map_df(1:5, function(i) {
  t_f <- train_sf[train_sf$fold_id != i, ]; v_f <- train_sf[train_sf$fold_id == i, ]
...
# Guardado de artefactos finales Fase III-A
...
saveRDS(resultados_cv,       "datos/kriging_cv.rds")
